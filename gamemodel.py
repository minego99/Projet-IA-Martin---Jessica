...
#joueur IA
class IA(Player):
    def __init__(self, epsilon = 0.9, learning_rate = 0.1, historic = [], previous_state = None ):
        self.epsilon = epsilon
        self.learning_rate = learning_rate
        self.historic = historic
        self.previous_state = previous_state
        self.value_function = {"lose":-1, "win":1}
        for(value in self.game.nb_matches):
            self.value_function = {value : 0}
            
    
    """
    Classe de l'IA avec apprentissage par renforcement
    
    
    """
   def exploit(self, matches_left):
       returned_value = value_function{previous_state} + learning_rate*(previous_state +  * ())
        
    def play(self):
        if(self.previous_state != None):
            self.historic.append(self.previous_state)
            
        self.previous_state = self.
        if(self.epsilon < random.range(0,1)):
         #   return exploit()
        else:
            #exploration
            return random.choice(1,2,3)
